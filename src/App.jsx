...
const GLOBAL_CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  :root{--bg:#0a0a0b;--card:#111113;--card-h:#18181b;--border:rgba(255,255,255,0.06);--muted:rgba(255,255,255,0.38);--dim:rgba(255,255,255,0.18)}
...
// ─── Static fallback data (shown while RSS loads) ─────────────────────────────
const BLUE_POSTS = [
  { id:"bp1", blue:true, game:"WoW", source:"Blizzard · BlueTracker", time:"loading...", tags:["Tuning","Classes"],   url:BT_HOME, title:"Class Tuning Incoming – March 31",             excerpt:"We will be applying the following tuning changes with scheduled maintenance on Monday, March 31." },
  { id:"bp2", blue:true, game:"WoW", source:"Blizzard · BlueTracker", time:"loading...", tags:["Items","Crafting"],   url:BT_HOME, title:"Recraft Myth Crests Are Gone",                  excerpt:"Recrafted items can no longer consume Myth Crests due to an unintended interaction." },
  { id:"bp3", blue:true, game:"WoW", source:"Blizzard · BlueTracker", time:"loading...", tags:["Event","Community"],  url:BT_HOME, title:"Play with the Blues: Void Assaults Stress Test" },
  { id:"bp4", blue:true, game:"WoW", source:"Blizzard · BlueTracker", time:"loading...", tags:["Weekly","News"],      url:BT_HOME, title:"WoW Weekly: Midnight Season 1, Twitch Drop, and More!" },
  { id:"bp5", blue:true, game:"WoW", source:"Blizzard · BlueTracker", time:"loading...", tags:["Hotfix"],             url:BT_HOME, title:"World of Warcraft: Midnight Hotfixes – March 26" },
  { id:"bp6", blue:true, game:"WoW", source:"Blizzard · BlueTracker", time:"loading...", tags:["Hotfix"],             url:BT_HOME, title:"World of Warcraft: Midnight Hotfixes – March 25" },
  { id:"bp7", blue:true, game:"WoW", source:"Blizzard · BlueTracker", time:"loading...", tags:["PTR","Development"],  url:BT_HOME, title:"12.0.5 PTR Development Notes" },
];

const WOW_ARTICLES = [
  { id:"w1", blue:false, game:"WoW", source:"Ruse's Bakery", time:"6 days ago", tags:["M+","Builds"], featured:true, url:null,
    title:"Mythic+ Season 1 Builds Now Live on Ruse's Bakery",
    excerpt:"Season 1 Midnight is live for Mythic+! Now it is time to gear up and chase those ratings!",
  ...
  { id:"w2", blue:false, game:"WoW", source:"Wowhead",       time:"2 days ago", tags:["Gear","BiS"],
    url:"https://www.wowhead.com/guide/best-in-slot",
...
const DOTA_ARTICLES = [
  { id:"d1", blue:false, game:"Dota 2", source:"Dota 2 Official", time:"3 days ago", tags:["Patch Notes"], featured:true,
    ...
    title:"Patch 7.38 – The Frontiers Update: Kez & Full Balance Notes",
    excerpt:"Patch 7.38 drops with brand-new hero Kez and sweeping jungle camp adjustments.",
...
function NewsCard({ article, index }) {
  ...
  return (
    <a href={isExt ? article.url : undefined} target={isExt ? "_blank" : undefined}
       ...
       style={{ textDecoration:"none",display:"block",animation:`fadeUp .4s ${index*52}ms ease both`,opacity:0,cursor:isExt?"pointer":"default" }}
...
function clockStr(date) { return date.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}); }
...
async function fetchBlueTracker() {
  ...
  try {
    ...
    if(res.ok){const r=parseRSSXml(await res.text());if(r?.length){writeCache(r);return r;}}
  } catch {}
  try {
    const res=await withTimeout(fetch(`https://api.rss2json.com/v1/api.json?rss_url=${enc}&count=30`),9000);
    if(res.ok){const j=await res.json();if(j.status==="ok"&&j.items?.length){const r=j.items.map((it,i)=>shapeItem(i,{title:it.title??"",link:it.link??"",pubDate:it.pubDate??"",description:it.description??it.content??""}));writeCache(r);return r;}}
  } catch {}
  try {
...
function NewsSection({ tab, setTab }) {
  ...
  useEffect(()=>{ const iv=setInterval(()=>setTickAge(a=>a+1),1000); return()=>clearInterval(iv); },[]);

  const ageLabel = lastFetched ? relTime(lastFetched).toUpperCase() : "…";
  const nextSec  = lastFetched ? Math.max(0,Math.round(REFRESH_MS/1000-tickAge)) : null;
  const nextLabel= nextSec!==null ? (nextSec>60?`next in ${Math.ceil(nextSec/60)}m`:`next in ${nextSec}s`) : "";

  const bluePosts = (livePosts??BLUE_POSTS).map(a=>a._ts?{...a,time:relTime(a._ts)}:a);
  const allArts   = [...bluePosts,...WOW_ARTICLES,...DOTA_ARTICLES];
  const articles  = allArts.filter(a=>tab==="all"||( tab==="wow"&&a.game==="WoW")||(tab==="dota"&&a.game==="Dota 2")||(tab==="blue"&&a.blue));
    ...
    <section style={{ marginBottom:52 }}>
      <div className="fu" style={{ display:"flex",flexDirection:"column",alignItems:"center",marginBottom:28 }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",justifyContent:"center" }}>
          ...
          {fetchError && <a href={BT_HOME} target="_blank" rel="noopener noreferrer" style={{ fontSize:9,padding:"2px 10px",borderRadius:6,background:"rgba(239,68,68,.1)",border:"1px solid rgba(239,68,68,.2)",color:"#f87171",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,letterSpacing:".1em",textDecoration:"none" }}>⚠ PROXY FAILED — VIEW ON BLUETRACKER.GG</a>}
          <button onClick={()=>doFetch(true)} disabled={fetching} style={{ background:"none",border:"none",cursor:fetching?"default":"pointer",color:"var(--dim)",display:"flex",alignItems:"center",gap:4,fontSize:11,fontFamily:"'Rajdhani',sans-serif",fontWeight:600,letterSpacing:".06em",padding:"3px 8px",borderRadius:8,transition:"color .2s,background .2s",opacity:fetching?.5:1 }}
      ...
      {lastFetched && (
        <div style={{ marginTop:16,display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>
          ...
          <a href={BT_HOME} target="_blank" rel="noopener noreferrer" style={{ fontSize:10,color:"rgba(80,150,255,.45)",fontFamily:"'Rajdhani',sans-serif",fontWeight:600,textDecoration:"none",letterSpacing:".04em",transition:"color .2s" }}
             onMouseEnter={e=>e.currentTarget.style.color="rgba(125,185,255,.8)"} onMouseLeave={e=>e.currentTarget.style.color="rgba(80,150,255,.45)"}>bluetracker.gg ↗</a>
        </div>
      )}
    </section>
  );
