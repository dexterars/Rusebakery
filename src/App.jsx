...
// ─── Video Background ─────────────────────────────────────────────────────────
//
//  YouTube cinematic videos cycle every 90 s with a smooth crossfade.
//  Only active on the WoW and Dota 2 tabs — other tabs use the canvas particles.
//
//  HOW TO ADD MORE VIDEOS:
//  1. Open a YouTube cinematic you like
//  2. Copy the 11-character ID from the URL bar (e.g. youtube.com/watch?v=XXXXXXXXXXX)
//  3. Paste it into the array below for the right game
//

const VIDEO_PLAYLISTS = {
  wow: [
    // ✅ The War Within | "Shadows Beneath" — Official Cinematic (Jul 2024)
    "zYdFLUBjwCU",
    // ✅ The War Within | Announce Cinematic "Echoes of the World Soul" (BlizzCon 2023)
    "hc9lqT6ZmGw",
    // ✅ Dragonflight | Official Announce Cinematic
    "AQG5p8i_oj8",
    // ✅ WoW Shadowlands | Official Cinematic "Beyond the Veil"
    "TjGHNnsoRtg",
  ],
  dota: [
    // ✅ The International 2024 — Opening Ceremony cinematic
    "j_-1_s4hMrE",
    // ✅ Dota 2 — "Are You Ready?" Animated Trailer
    "R6BaxKyNi2M",
    // ✅ Dota 2 | Crownfall Act IV — Kez Reveal Cinematic
    "UuQ0IaRV7EQ",
    // ✅ Dota 2 — Official Gameplay Trailer
    "pX0sefmk5OA",
  ],
};

// Rotate interval in ms (90 seconds per video)
const VIDEO_ROTATE_MS = 90_000;

function VideoBackground({ tab, bgOpacity }) {
  const [vidIdx, setVidIdx]   = useState(0);
  const [visible, setVisible] = useState(true); // for crossfade

  const playlist = VIDEO_PLAYLISTS[tab] ?? [];

  // Reset to first video when tab changes
  useEffect(() => {
    setVidIdx(0);
    setVisible(true);
  }, [tab]);

  // Cycle videos every 90 seconds with 1 s crossfade
  useEffect(() => {
  ...
  const videoId = playlist[vidIdx];
  // YouTube embed params: autoplay, muted, looped, no controls/branding
  const src = [
    ...
    <div style={{
      ...
      <iframe
        ...
        style={{
          ...
          height: "56.25vw",   /* 9:16  → 100vw × 9/16 */
          minWidth:  "100%",
          minHeight: "100%",
          pointerEvents: "none",
          border: "none",
      ...
      {/* Next video button — only clickable */}
      {playlist.length > 1 && (
        <button
          onClick={() => {
            setVisible(false);
          ...
          style={{
            ...
            zIndex: 9, pointerEvents: "all", cursor: "pointer",
        ...
        >
          ▶ NEXT VIDEO ({vidIdx + 1}/{playlist.length})
        </button>
      )}
    </div>
  );
...
function AnimatedBackground({ theme, tab, bgOpacity }) {
  ...
  return (
    <>
      ...
      {/* Video (wow/dota) OR static wallpaper (blue) */}
      {(tab === "wow" || tab === "dota") ? (
        <VideoBackground tab={tab} bgOpacity={bgOpacity} />
      ) : (theme.image && (
